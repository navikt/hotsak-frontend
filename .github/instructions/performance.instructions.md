...
export default async function Page() {
  const [users, metrics, config] = await Promise.all([fetchUsers(), fetchMetrics(), fetchConfig()])
...
const getCachedMetrics = unstable_cache(async () => fetchMetrics(), ['metrics'], { revalidate: 300, tags: ['metrics'] })
```
