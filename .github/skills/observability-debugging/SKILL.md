...
| ----------- | ------------------ | --------------------------------------------------- | ------------------------------ |
| **Metrics** | Mimir (Prometheus) | _What_ is happening? (rates, quantiles, saturation) | `mimir.nav.cloud.nais.io`      |
| **Logs**    | Loki               | _Why_ is it happening? (errors, context, messages)  | `loki.nav.cloud.nais.io`       |
| **Traces**  | Tempo              | _Where_ in the call chain? (latency, dependencies)  | `tempo.$env.nav.cloud.nais.io` |
