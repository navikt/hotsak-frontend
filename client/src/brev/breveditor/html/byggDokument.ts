...
export function byggHeaderHtml(metadata: Metadata, vergeNavn?: string): string {
  return (
    `<header class="header">` +
    NAV_LOGO_SVG +
    `<div class="header-row">
        <dl>
          ${vergeNavn ? `<dt>Verge:</dt><dd>${vergeNavn}</dd>` : ''}
          <dt>${vergeNavn ? 'Saken gjelder' : 'Navn'}:</dt>
...
export function byggFullHtml(metadata: Metadata, nyVerdi: Value, vergeNavn?: string): string {
  return (
    `<html lang="nb">
        <head>
          <meta charset="UTF-8">
        ...
        <body>` +
    byggHeaderHtml(metadata, vergeNavn) +
    `<main>` +
