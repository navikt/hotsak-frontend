import useSWRImmutable from 'swr/immutable'
import { Målform } from '../brevTyper.ts'

function loadTemplate(key: string): Promise<{ default: string }> {
  switch (key) {
    case 'innvilgelse':
      return import('./maler/innvilgelse.md?raw')
    case 'innvilgelse-nn':
      return import('./maler/innvilgelse-nn.md?raw')
    case 'delvis-innvilgelse-bruker-har-ikke-rett':
      return import('./maler/delvis-innvilgelse-bruker-har-ikke-rett.md?raw')
    case 'delvis-innvilgelse-bruker-har-ikke-rett-nn':
      return import('./maler/delvis-innvilgelse-bruker-har-ikke-rett-nn.md?raw')
    case 'delvis-innvilgelse-hjelpemiddelet-gis-ikke':
      return import('./maler/delvis-innvilgelse-hjelpemiddelet-gis-ikke.md?raw')
    case 'delvis-innvilgelse-andre-enn-nav-dekker':
      return import('./maler/delvis-innvilgelse-andre-enn-nav-dekker.md?raw')
    case 'avslag-bruker-har-ikke-rett':
      return import('./maler/avslag-bruker-har-ikke-rett.md?raw')
    case 'avslag-bruker-har-ikke-rett-nn':
      return import('./maler/avslag-bruker-har-ikke-rett-nn.md?raw')
    case 'avslag-hjelpemiddelet-gis-ikke':
      return import('./maler/avslag-hjelpemiddelet-gis-ikke.md?raw')
    case 'avslag-andre-enn-nav-dekker':
      return import('./maler/avslag-andre-enn-nav-dekker.md?raw')
    case 'henleggelse':
      return import('./maler/henleggelse.md?raw')
    case 'henleggelse-nn':
      return import('./maler/henleggelse-nn.md?raw')
    case 'svartidsbrev':
      return import('./maler/svartidsbrev.md?raw')
    case 'innhente-opplysninger':
      return import('./maler/innhente-opplysninger.md?raw')
    default:
      return Promise.resolve({ default: '' })
  }
}

async function templateFetcher(key: string) {
  const template = await loadTemplate(key)
  return template.default
}

// Nynorsk-maler har samme nøkkel som bokmål-malen, med "-nn"-suffiks, f.eks. "innvilgelse-nn".
function malnøkkelFor(key: string, målform?: Målform): string {
  return målform === Målform.NYNORSK ? `${key}-nn` : key
}

export function useBrevmal(key?: string, målform?: Målform) {
  const malnøkkel = key ? malnøkkelFor(key, målform) : undefined
  const { data } = useSWRImmutable(malnøkkel || null, templateFetcher, {
    suspense: true,
  })
  return data
}
