...
export const saksbehandlingHandlers: StoreHandlersFactory = ({
  ...
  async function oppdaterNotatGjenstående(sakId: string, behandlingId: number) {
    const notater = await notatStore.hentNotater(sakId)
    const behandling = await sakStore.hentBehandling(behandlingId)
    if (!behandling) return

    const harNotatutkast = notater.some(isNotatUtkast)
    const gjenstående: Gjenstående[] = behandling.gjenstående.filter((g) => g !== Gjenstående.NOTAT_IKKE_FERDIGSTILT)
    if (harNotatutkast) {
      gjenstående.push(Gjenstående.NOTAT_IKKE_FERDIGSTILT)
    }

    await sakStore.oppdaterBehandling(behandlingId, { ...behandling, gjenstående })
  }
  ...
  return [
    ...
    http.post<SakParams, LagreBehandlingRequest, Behandling>(
      ...
      async ({ params, request }) => {
        ...
        await oppdaterNotatGjenstående(params.sakId, Number(behandlingId))
    ...
    http.put<BehandlingParams, LagreBehandlingRequest>(
      ...
      async ({ params, request }) => {
        ...
        await oppdaterNotatGjenstående(params.sakId, Number(params.behandlingId))
        return respondNoContent()
      }
    ),
    ...
    http.post<BehandlingParams>('/api/sak/:sakId/behandling/:behandlingId/angring', async ({ params }) => {
      ...
      const behandlingerForSak = await sakStore.hentBehandlinger(sakId)
      const gjeldendeBehandling = behandlingerForSak[0]!
      // TODO: Implement angre vedtak logic
